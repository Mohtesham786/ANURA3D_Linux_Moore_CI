...
! use kernel32
   use ModMeshInfo
   use ModLinearElasticity, only: ESM_LinearElasticity ! Linear elasticity model
   use MOD_MCSS_ESM       , only: ESM_MCSS             ! Mohr COulomb Strain softening model
  !  use MOD_MC_ESM         , only: ESM_MC               ! Mohr-Coulomb model
...
!*********************************************************************
      ...
      real(REAL_TYPE) :: shape_factor ! Shape factor value for MCSS ESM when being used for MC 
      ...
      if (trim(NameModel)//char(0) == trim('linear_elasticity')//char(0)) then
         ...
         ! Call the ESM for linear elasticity
         call ESM_LinearElasticity(IDpt, IDel, IDset, Stress, Eunloading, PlasticMultiplier, StrainIncr, NSTATEVAR, StateVar, nAddVar, AdditionalVar,cmname, NPROPERTIES, props, CalParams%NumberOfPhases, ntens)
      elseif (trim(NameModel)//char(0) == trim(ESM_MOHR_COULOMB_STANDARD)//char(0)) then
         ! Note: MC model not implemented at this time
         print *, "MC model Not implemented"
      ...
      elseif(NameModel == ESM_ARB_Model_MohrCoulombStrainSoftening) then
          ...
          call ESM_MCSS(IDpt, IDel, IDset, Stress, Eunloading, PlasticMultiplier, StrainIncr, NSTATEVAR, StateVar,&
  ...
  !  p = GetProcAddress(MatParams(IDSet)%SoilModelDLLHandle, "ESM"C) ! Pointing to the ESM .dll
   ...
   subroutine CalculateViscousDamping_interface(ParticleID, IEl)
    ...
    !*************************************************************************************
    !    SUBROUTINE: set_props_4_MC
    ! 
    !    DESCRIPTION:
    !>   Currenly the Mod_MCSS_ESM is used for running regular Mohr-Coloumb (MC) models. 
    !>   This subroutine formats the regular MC input properties to match the format of the ESM_MCSS subroutine
    !
    !>   @note : Created by WaveHello. This isn't the cleanest way to do this. However it will work for linux testing
    !            Probably better to make a copy of the ESM_MCSS, remove the strain softening capability and 
    !
    ...
    !
    !*************************************************************************************
   subroutine set_props_4_MC(particle, material_params, props)
    
    type(ParticleType(NTENSOR, NVECTOR, MAX_LOAD_SYSTEMS))  , intent(in)    :: particle
    type(MaterialParameterType)                             , intent(in)    :: material_params
    real(REAL_TYPE), dimension(:)                           , intent(inout) :: props

    ! Local variables
    real(REAL_TYPE) :: cohesion, friction_angle

    ! Unpack the friction angle for calculations and for setting prop values
    friction_angle = material_params%FrictionAngle

    ! --- Start setting prop values --- 

    props(1) = particle%ShearModulus ! shear modulus, G
    props(2) = material_params%PoissonRatio
    
    ! Calc sin (friction_angle) for the purpose of back converting cohesion
    sin_phi = SIN(friction_angle * Pi/180.0)

    ! Convertfrom CohesionCosPhi to regular cohesion
    cohesion = particle%CohesionCosPhi / dsqrt(1D0 - sin_phi * sin_phi) 
    
    ! Setting the values of cohesion for peak and residual
    props(3) = cohesion
    props(4) = cohesion
     
    ! Set the residual and peak friction angle
    props(5) = friction_angle
    props(6) = friction_angle
    
    ! Set the dilation angle
    props(7) = material_params%DilatancyAngle
    props(8) = material_params%DilatancyAngle
    
    shape_factor = 0.0
    props(9) = shape_factor
    
    ! Set the integration to Ortiz-Simo, 0 for Abbo-Sloan
    props(10) = 1

    ! Yield Surface tolerance
    props(11) = 1e-8

    ! Max number of integration steps allowed
    props(12) = 10

    ! Set Abbo and Sloan DT min to arbitrary value as it isn't used
    ! FIXME: THis solution isn't nice 
    props(13) = 999 
    
   end subroutine set_props_4_MC
