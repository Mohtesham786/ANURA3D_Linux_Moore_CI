    ...
    !*****************************************************************************
      ...
      contains
        ...
        implicit none
          ...
          if ( .not.( mod(CalParams%TimeStep, CalParams%OutputCurvesIntervals)==0 .or. CalParams%TimeStep==1 .or. IsLastTimeStepOfExplicitCalculation() ) ) RETURN
              ...
              if (CalParams%OutputParticles(I)>0) then
               if (CalParams%ApplyExcavation) then                 
                ...
                if (ParticleIndex<=Counters%NParticles) then
                  ...
                  ! Printing the time step to see if the problem is in opening and writing or if the subroutine is only being called at the 
                  ! end of the loadstep

                  print *, "Time Step", CalParams%TimeStep
